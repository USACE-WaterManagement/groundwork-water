...
export function useNearestSeriesData({
  ...
  // this window, the recent-value fallback below kicks in.
  ...
  // the series' last value, causing useCdaMultiTimeSeries to re-fetch those
  // series. Only the timestamp is used here - the value itself comes back from
  // that windowed fetch, so the selection rules stay in one place.
  useEffect(() => {
    ...
    for (const entry of seriesNeedingLatest) {
      ...
      newDates[seriesKey(entry.tsid, entry.units)] = new Date(ms).toISOString();
    }
  ...
  }, [recentResult.data, seriesNeedingLatest]);
