...
// Shared empty objects keep unpinned renders referentially stable.
const EMPTY_DATES = {};
...
export function useNearestSeriesData({
  ...
  // Keep windows per-series so one slow-reporting gate does not widen the
  // request for every other column in the form.
  ...
  // Pins are scoped to the window that produced them. Changing the calendar
  // or lookback must make the initial request follow the new target again.
  ...
  const tsParams = useMemo(() => {
    ...
    return series.map(({ tsid, units }) => {
      ...
      return {
        name: tsid,
        office,
        // Current cwmsjs maps `units` to CDA's canonical plural parameter.
        units,
        begin: pin
  ...
  const seriesNeedingExtents = useMemo(() => {
    if (!tsDone) return [];
    return series.filter((entry, i) => {
      if (latestDates[seriesKey(entry.tsid, entry.units)]) return false;
      const values = tsResults[i]?.data?.values;
      return !values || values.every((entry) => entry[1] === null);
    });
  }, [tsDone, tsResults, series, latestDates]);

  const extentTsids = useMemo(
    () => [...new Set(seriesNeedingExtents.map((entry) => entry.tsid))],
    [seriesNeedingExtents],
  );

  const extentsResult = useCdaTimeSeriesExtents({
    tsIds: extentTsids,
    office,
    cdaUrl,
    enabled: shouldFetch && extentTsids.length > 0,
    staleTime,
  });

  // Re-read around an all-time latest extent only when the series ended before
  // the requested window. If the extent is newer, it cannot identify the value
  // immediately before a historical target.
  useEffect(() => {
    const newDates = {};
    for (const entry of seriesNeedingExtents) {
      const key = seriesKey(entry.tsid, entry.units);
      const iso = extentsResult.data[entry.tsid];
      ...
      newDates[key] = new Date(latestMs).toISOString();
    }
  ...
  }, [extentsResult.data, seriesNeedingExtents, baseWindows, windowKey]);

  const isPending =
    shouldFetch &&
    (!tsDone || (seriesNeedingExtents.length > 0 && extentsResult.isPending));
  ...
  const error = useMemo(() => {
    ...
    return failed?.error ?? extentsResult.error ?? null;
  }, [tsResults, extentsResult.error]);
