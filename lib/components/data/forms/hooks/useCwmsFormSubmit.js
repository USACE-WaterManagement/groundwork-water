...
import { TimeSeriesApi, TextTimeSeriesApi, BlobApi, Configuration } from "cwmsjs";
import { buildCwmsDataUploadPayloads } from "../helpers/dataUpload";
...
export function useCwmsFormSubmit({
  ...
  const createApiConfig = (inputCdaUrl = cdaUrl) => {
    const configParams = {
      basePath: inputCdaUrl,
      headers: {
  ...
  const mutation = useMutation({
    mutationFn: async (formInputs) => {
      ...
      try {
        ...
        const blob_api = new BlobApi(config);

        const results = [];
        const errors = [];

        // Process each input with TSID, blob, or a parsed tabular upload.
        for (const input of formInputs) {
          if (input.kind === "timeseries-batch") {
            const batchConfig = input.cdaUrl ? createApiConfig(input.cdaUrl) : config;
            const batchTsApi = new TimeSeriesApi(batchConfig);
            const batchTextApi = new TextTimeSeriesApi(batchConfig);
            const payloads = buildCwmsDataUploadPayloads(input.rows || [], {
              unit: input.unit,
              valueUrlBase: input.cdaUrl || cdaUrl,
            });

            for (const timeSeries of payloads.numeric) {
              try {
                await batchTsApi.postTimeSeries({
                  timeSeries,
                  timezone: "UTC",
                  createAsLrts: false,
                  storeRule: input.storeRule || storeRule,
                  overrideProtection: input.overrideProtection ?? true,
                });
                results.push({
                  tsid: timeSeries.name,
                  type: "numeric-batch",
                  status: "success",
                  valueCount: timeSeries.values.length,
                });
              } catch (error) {
                errors.push({
                  tsid: timeSeries.name,
            ...
            for (const textTimeSeries of payloads.text) {
              try {
                await batchTextApi.postTimeSeriesText({
                  textTimeSeries,
                  replaceAll: true,
                });
                results.push({
                  tsid: textTimeSeries.name,
                  type: "text-batch",
                  status: "success",
                  valueCount: textTimeSeries.regularTextValues.length,
                });
              } catch (error) {
                errors.push({
                  tsid: textTimeSeries.name,
          ...
          } else if (input.kind === "blob" || input.blobId) {
            try {
              const rawValue = input.value ?? input.values?.[0];

              if (rawValue) {
                await blob_api.postBlobs({
                  blob: {
                    officeId: input.officeId || office,
                    id: input.blobId,
    ...
    onSuccess: (data, variables) => {
      ...
      if (variables) {
        const submittedTsids = [
          ...new Set(
            variables.flatMap((input) =>
              input.kind === "timeseries-batch"
                ? (input.rows || []).map((row) => row.tsid).filter(Boolean)
                : input.tsid
                  ? [input.tsid]
                  : [],
            ),
        ...
        const submittedBlobIds = variables
          .filter((input) => input.blobId)
          .map((input) => input.blobId);
        ...
        submittedTsids.forEach((tsid) => {
          queryClient.invalidateQueries({
            predicate: (query) =>
              ...
              ["timeseries", "data-upload"].includes(query.queryKey[1]) &&
              query.queryKey.includes(tsid),
          });
        });

        submittedBlobIds.forEach((blobId) => {
          queryClient.invalidateQueries({
            predicate: (query) =>
              query.queryKey[0] === "cda" &&
              query.queryKey[1] === "blob" &&
              query.queryKey.includes(blobId),
          });
        });

        if (submittedBlobIds.length > 0) {
          queryClient.invalidateQueries({
            predicate: (query) =>
              query.queryKey[0] === "cda" && query.queryKey[1] === "blobs",
          });
        }
      }
...
export function useFormValidation() {
  const validateInputs = (inputs) => {
    ...
    inputs.forEach((input) => {
      if (input.validate) {
        const customValidation = input.validate();
        if (customValidation) {
          validationErrors.push({
            name: input.name || input.tsid || input.blobId,
            message:
              typeof customValidation === "string"
                ? customValidation
                : customValidation.message ||
                  `${input.label || input.name || input.tsid || input.blobId || "Field"} is invalid`,
            input: input,
          });
          return;
        }
