...
const SEED_VALUE_TOLERANCE = 1e-12;

export function seedValuesMatch(fetched, seeded) {
  if (fetched === seeded) return true;
  if (
    fetched === null ||
    fetched === undefined ||
    seeded === null ||
  ...
  const fetchedNumber = Number(fetched);
  const seededNumber = Number(seeded);
  if (!Number.isFinite(fetchedNumber) || !Number.isFinite(seededNumber)) return false;

  const scale = Math.max(1, Math.abs(fetchedNumber), Math.abs(seededNumber));
  return Math.abs(fetchedNumber - seededNumber) <= scale * SEED_VALUE_TOLERANCE;
}
...
function buildSpecKey({
  ...
  (columns ?? []).forEach((column) => {
    ...
    // Lookback is a property of the series, not of a cell: every row of a
    // column shares one fetch window, so only column and form settings apply.
    seen.set(
      key,
      mergeSeriesWindows(seen.get(key), {
        tsid: column.tsid,
        units,
  ...
  const plan = useMemo(() => {
    ...
    for (const specKey of needsRef.current.keys()) {
      ...
      spec.series?.forEach((entry) => {
        ...
        // Two components can want the same series over different windows; the
        // shared request has to cover the wider of them.
        seriesMap.set(key, mergeSeriesWindows(seriesMap.get(key), entry));
      });
  ...
  useEffect(() => {
    ...
    seeds.forEach((forSeries, key) => {
      ...
      forSeries.forEach((value, ms) => {
        if (fetchedAt.has(ms) && seedValuesMatch(fetchedAt.get(ms), value))
          changed = true;
...
export function useNearestValues({
  ...
  // A component's own setting wins; otherwise the form's default applies. Doing
  // this here rather than in each component means every input that declares a
  // need honours the form-level setting, including ones wired up later.
  const effectiveLookback = lookback ?? context.lookback;
  const effectiveLookahead = lookahead ?? context.lookahead;
