...
/**
 * Timezone-aware snap using a dayjs instance.
 * Works correctly in any timezone because all accessors (.minute(), .hour(), etc.)
 * operate in whatever zone the dayjs object is set to (local, UTC, or a named zone).
 *
 * @param {import("dayjs").Dayjs} d - A dayjs instance (can be local, utc, or tz)
 * @param {string} interval - The interval to snap to
 * @param {string} snapTo - The direction to snap (nearest, previous, next)
 * @returns {import("dayjs").Dayjs} - The snapped dayjs instance
 */
export function snapDayjs(d, interval, snapTo = "nearest") {
  switch (interval) {
    case "second":
      return d.millisecond(0);

    case "minute": {
      let r = d.second(0).millisecond(0);
      if (snapTo === "next") r = r.add(1, "minute");
      return r;
    }
    ...
    case "5minutes": {
      ...
      const rem = mins % 5;
      if (snapTo === "nearest") {
        r = rem >= 3 ? r.minute(mins + (5 - rem)) : r.minute(mins - rem);
      } else if (snapTo === "next") {
        r = r.minute(mins + (5 - rem));
      } else {
        r = r.minute(mins - rem);
      }
      return r;
    ...
    case "15minutes": {
      ...
      const rem = mins % 15;
      if (snapTo === "nearest") {
        r = rem >= 8 ? r.minute(mins + (15 - rem)) : r.minute(mins - rem);
      } else if (snapTo === "next") {
        r = r.minute(mins + (15 - rem));
      } else {
        r = r.minute(mins - rem);
      }
      return r;
    ...
    case "30minutes": {
      ...
      const rem = mins % 30;
      if (snapTo === "nearest") {
        r = rem >= 15 ? r.minute(mins + (30 - rem)) : r.minute(mins - rem);
      } else if (snapTo === "next") {
        r = r.minute(mins + (30 - rem));
      } else {
        r = r.minute(mins - rem);
      }
      return r;
    ...
    case "hour": {
      const curMins = d.minute();
      let r = d.minute(0).second(0).millisecond(0);
      if (snapTo === "nearest" && curMins >= 30) {
        r = r.add(1, "hour");
      } else if (snapTo === "next") {
        r = r.add(1, "hour");
      }
    ...
    case "day": {
      const curHours = d.hour();
      let r = d.hour(0).minute(0).second(0).millisecond(0);
      if (snapTo === "nearest" && curHours >= 12) {
        r = r.add(1, "day");
      } else if (snapTo === "next") {
        r = r.add(1, "day");
      }
    ...
    case "month": {
      ...
      let r = d.date(1).hour(0).minute(0).second(0).millisecond(0);
      if (snapTo === "nearest" && curDay > daysInMonth / 2) {
        r = r.add(1, "month");
      } else if (snapTo === "next") {
        r = r.add(1, "month");
      }
    ...
    case "year": {
      ...
      let r = d.month(0).date(1).hour(0).minute(0).second(0).millisecond(0);
      if (snapTo === "nearest" && curMonth > 5) {
        r = r.add(1, "year");
      } else if (snapTo === "next") {
        r = r.add(1, "year");
      }
