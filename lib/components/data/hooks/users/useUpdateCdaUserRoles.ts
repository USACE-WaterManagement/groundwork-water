...
export interface UpdateCdaUserRolesInput {
  userName: string;
  office: string;
...
export interface UseUpdateCdaUserRolesParams {
  cdaUrl: string;
  token?: string;
}

export function diffCdaUserRoles(previousRoles: string[], roles: string[]) {
  const previous = new Set(previousRoles);
  const next = new Set(roles);
  const additions = roles.filter((role) => !previous.has(role));
  const removals = previousRoles.filter(
    (role) => role !== "All Users" && !next.has(role),
  );
...
export const useUpdateCdaUserRoles = ({
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { additions, removals } = diffCdaUserRoles(previousRoles, roles);
