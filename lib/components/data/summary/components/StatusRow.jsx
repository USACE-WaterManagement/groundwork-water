...
export default function StatusRow({
  ...
  const {
    ...
    queryKey: ["dataStatusTS", tsid, dayjs().format("YYYY-MM-DDTHH:mm:ss")],
    queryFn: async () => {
  ...
  useLayoutEffect(() => {
    if (tsData?.values)
      setStatusDelta(
        Math.round(
          cellRef.current?.getBoundingClientRect()?.width / tsData?.values.length,
        ),
      );
  }, [tsPending]);
  if (!office) {
      ...
      {tsPending ? (
        ...
        <>
          <TableCell title={name}>
            {linkPath ? (
              <TextLink href={`${linkPath}/${tsid.split(".")[0]}`}>
                {`${tsid.split(".")[0]} ${tsid.split(".")[1]}`}
              </TextLink>
            ) : (
              `${tsid.split(".")[0]} ${tsid.split(".")[1]}`
            )}
          ...
          <TableCell>
            <svg ref={cellRef} key={`svg-${tsid}`} width="100%" height="10">
              {tsData?.values
                ? tsData?.values.map((val, idx) => {
                    return (
                      <line
                        ...
                        data-tooltip={
                          dayjs(val[0]).format(dateFormat) + " " + val[1]?.toFixed(2)
                        }
                      />
                    );
                  })
