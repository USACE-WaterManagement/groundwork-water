import { Text, TextLink, TableRow, TableCell, Skeleton } from "@usace/groundwork";
...
export default function StatusRow({ office, linkPath, name, pageSize, lookBackHours, cdaUrl, dateFormat }) {
    ...
    const { data: tsData, error: tsError, isPending: tsPending } = useQuery({
        queryKey: ["dataStatusTS", name, dayjs().format("YYYY-MM-DDTHH:mm:ss")],
        queryFn: async () => {
            return tsApi.getTimeSeries({
                office,
                name: name,
                ...
                if (data?.name && data?.values) {
                    ...
                    if (data.values.length > 0) {
                        ...
                        if (first_dt > lookBack.valueOf()) {
                            const missing_count = dayjs(first_dt).diff(lookBack, "hours", true) * 4
    ...
    useLayoutEffect(() => {
        if (tsData?.values?.[1]?.[0])
            setStatusDelta(Math.round(cellRef.current?.getBoundingClientRect()?.width / tsData?.values[1][0].length));
    }, [tsPending])
    if (tsError) {
        return <TableRow><TableCell>Error: {tsError.message}</TableCell></TableRow>
    }
    return (<TableRow>
        {tsPending ? <TableCell colSpan={3}><Skeleton className="gww-w-100" /></TableCell> :
            <>
                <TableCell title={name}>
                    {linkPath ? <TextLink
                        href={`${linkPath}/${name.split(".")[0]}`}>
                        {`${name.split(".")[0]} ${name.split(".")[1]}`}
                    </TextLink> : `${name.split(".")[0]} ${name.split(".")[1]}`
                    }
                </TableCell>
                <TableCell>
                    <svg
                    ...
                    >
                        {(tsData?.values[1][0]) ?
                            tsData?.values[1][0].map((val, idx) => {
                                console.log({ idx })
                                return (
                                    <line key={`line-${name}-${idx}`}
                                        x1={Math.round(statusDelta * idx)} y1="0"
                                        x2={Math.round(statusDelta * idx)} y2="10"
                ...
                <TableCell>
                    {(tsData?.values?.[0]) ? dayjs(tsData.values[0]).format(dateFormat) : "Missing"}
                </TableCell>
            </>
        }
    </TableRow >
