  ...
  return (
    <DocsPage middleText="Data Status Overview">
      ...
      {!filePending && fileError ? (
        <div>Failed to load TSID data status file! {fileError?.message}</div>
      ) : (
...
/* nae.datastatus Contents : 
: This is a comment
UVD.Stage-Pool.Inst.15Minutes.0.DCP-rev
UVD.Stage-TAILWATER.Inst.15Minutes.0.DCP-rev
: UVD.Precip.Inst.15Minutes.0.DCP-rev
*/
...
default export function Example() {
    ...
    const tsids = ["UVD.Stage-Pool.Inst.15Minutes.0.DCP-rev"]
    return <DataStatus
        office="NAE"
...
`}
      ...
      <Text className="mb-2">
        Use <Code>useDataStatusFile</Code> to load a newline-delimited TSID file and
        turn it into an array that can be passed into <Code>DataStatus</Code>. Lines
        beginning with <Code>:</Code> are treated as comments and ignored.
      </Text>
...
function Example() {
    ...
    return <DataStatus office="NAE" tsids={tsids} />;
}`}
      </Code>
      <div className="font-bold text-lg pt-6">
        Hook Parameters
