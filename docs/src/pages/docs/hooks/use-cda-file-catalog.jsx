...
import { useCdaFileCatalog } from "@usace-watermanagement/groundwork-water";
...
const FileCatalogCard = () => {
  ...
  const { data, isPending, isError } = useCdaFileCatalog({
    cdaParams: {
      office,
      like: debouncedLike,
    },
...
function UseCdaFileCatalog() {
  return (
    <DocsPage middleText="CDA File Catalog Hook">
      <div>
        <Text>
          {`The useCdaFileCatalog hook retrieves blob metadata using the
  ...
  return (
    <Card className="w-fit">
      ...
      <div className="font-bold text-lg pt-6">
        Hook Parameters -{" "}
        <Code className="p-2">{`useCdaFileCatalog({...})`}</Code>
      </div>
      <CdaParamsTable
        requestObject="Blob"
        requestType="GET"
