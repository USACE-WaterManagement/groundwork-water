    ...
    <DocsPage middleText="MIDAS Project Time Series Hook">
      <div>
        <Text>
          <div>
            <Text>
              <strong>MIDAS</strong> (Monitoring Instrumentation Data
              Acquisition System) is the U.S. Army Corps of Engineers&apos;
              centralized platform for storing and managing field
              instrumentation data at water resource projects such as dams,
              levees, and reservoirs.
            </Text>
            <Text className="mt-4">
              Each project in MIDAS may have one or more{" "}
              <strong>instruments</strong> (e.g. piezometers, GPS sensors,
              inclinometers), and each instrument can produce one or more{" "}
              <strong>time series</strong> — representing either measured or
              computed values over time.
            </Text>
            <Text className="mt-4">
              The <Code>useMidasProjectTS</Code> hook fetches the metadata for
              all time series associated with a specific instrument at a given
              project. This metadata includes the time series name, type
              (standard, computed, or constant), unit, parameter, and variable
              identifier — all of which are necessary for querying and
              visualizing data.
            </Text>
            <Text className="mt-4">
              This hook is useful when building UIs that allow users to select
              from available MIDAS time series or when integrating MIDAS data
              into Groundwork applications.
            </Text>
          </div>
        </Text>
        <QueryClientWarning />
  ...
  return (
    <ul>
      ...
      <div className="font-bold text-lg pt-6">
        Hook Parameters -{" "}
        <Code className="p-2">{`useMidasProjectTS({...})`}</Code>
      </div>
