...
import { useCdaFile } from "@usace-watermanagement/groundwork-water";
...
const FileViewerCard = () => {
  ...
  const { data, isPending, isError } = useCdaFile({
...
function UseCdaFile() {
  return (
    <DocsPage middleText="CDA File Hook">
      <Text>
        The <Code>useCdaFile</Code> hook retrieves a single file (blob) from the
  ...
  const FileViewerCard = () => {
    ...
    return (
      ...
      <div className="font-bold text-lg pt-6">
        Hook Parameters - <Code className="p-2">{`useCdaFile({...})`}</Code>
      </div>
      <CdaParamsTable
        requestObject="Blob"
        requestType="GET"
